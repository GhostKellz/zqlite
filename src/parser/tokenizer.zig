...
pub const Tokenizer = struct {
    ...
    fn readString(self: *Self, allocator: std.mem.Allocator) !Token {
        ...
        // Build the string content, collapsing SQL-standard escaped quotes
        // (a doubled quote `''` inside the literal represents a single `'`).
        ...
        while (self.current_char) |c| {
            if (c == quote_char) {
                ...
                // Otherwise this was the closing quote.
                const owned_string = try content.toOwnedSlice(allocator);
...
test "tokenizer trailing escaped quote" {
    const allocator = std.testing.allocator;
